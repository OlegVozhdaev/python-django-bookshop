...
from managebook.forms import CustomUserCreateForm
   ...
   # @method_decorator(cache_page(2))
    def get(self, request):
        ...
        if request.user.is_authenticated:
            query = Q(book_like__user_id=request.user.id)
            sub_query = Book.objects.filter(query).annotate(user_rate=Cast("book_like__rate", CharField())) \
                .prefetch_related("author", "genre", "comment", "comment__user")
            result = Book.objects.filter(~query).annotate(user_rate=Value(-1, CharField())) \
                .prefetch_related("author", "genre", "comment", "comment__user").union(sub_query)
        else:
            result = Book.objects \
